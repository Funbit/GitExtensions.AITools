...
function Find-ArchiveUrl
{
    ...
    if ($Source -eq "GitHub")
    {
        return Find-ArchiveUrlFromGitHub -Version $Version -ExpectedMajorVersion $ExpectedMajorVersion;
    }
...
function Find-ArchiveUrlFromGitHub
{
    ...
    {
        if ($ExpectedMajorVersion -gt 0)
        {
            Write-Host "Searching for latest release matching major version $ExpectedMajorVersion.";
            $Releases = Invoke-RestMethod -Uri $BaseUrl;
            foreach ($Release in $Releases)
            {
                $Tag = $Release.tag_name -replace '^v', '';
                $Major = ($Tag -split '\.')[0];
                if ($Major -eq $ExpectedMajorVersion)
                {
    ...
    if (!($null -eq $SelectedRelease))
    {
        ...
        {
            if ($Asset.name.Contains('Portable') -and $Asset.name.Contains('x64') -and $Asset.name.EndsWith('.zip'))
            {
...
try 
{
    ...
    $SelectedVersion,$DownloadUrl = Find-ArchiveUrl -Version $Version -Source $Source -ExpectedMajorVersion $ExpectedMajorVersion;
    if ($Version -eq $LatestVersionName) 
    {
